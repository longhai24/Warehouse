...
<label>论文题目:</label><input type="text" name="title" required="required" value="${PAPER.title }">
<span style="color: red;"></span>
 <br>
<label>类型</label>
<select name="cid" required="required">
<c:forEach items="${CODE_LIST }" var="c">
<option value="${c.id }"<c:if test="${PAPER.cid==c.id }">selected='selected'</c:if> />${c.codetypename}
</c:forEach>
</select>
 <br>
<label>论文摘要</label>
...
${PAPER.papersummary }
 ...
 <br>
<label>论文内容</label><span>${PAPER.filename }</span> <input type="file" name="filename2">
<span  style="color: red;"></span>
 <br>
