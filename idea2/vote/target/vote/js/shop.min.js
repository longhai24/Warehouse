    ...
    t.extend(i.prototype, {
        init: function () {
            ...
            a.settings.init && a.settings.init(), a.cart_empty(), a.product.each(function (e) {
                var n = t(this), i = n.find("." + a.settings.product_title), r = n.find("." + a.settings.product_price),
                    s = n.find("." + a.settings.product_button), c = i.text(),
            ...
            var t = this;
            t.cart.parent().append('<button class="' + t.settings.cart + '-clear">清空</button>'), t.cart.parent().append('<button type="submit" class="' + t.settings.cart + '-checkout">提交</button>'), t.clear_cart(), t.checkout_button()
        }, checkout: function () {
            var e, a = this, n = {vpid: vpid}, i = t("<form />"),
                r = {name: "item_name", quantity: "quantity", amount: "amount"};
            t.extend(n, a.settings.paypal), i.attr("action", "vote?src=addVoteogt"), i.attr("method", "post"), i.attr("target", "_blank");
            for (var s in n) e = t('<input type="hidden" />'), e.attr("name", s), e.attr("value", n[s]), i.append(e);
            a.cart.find("li").each(function (e) {
                var n = t(this), s = n.find("." + a.settings.cart + "-name").text(),
                    c = parseFloat(n.find("." + a.settings.cart + "-price").text().replace(/^\D+/g, "")),
                    o = n.find("input").val(), u = {
                        name: t("<input />", {type: "hidden", name: r.name + "_" + (e + 1), value: s}),
            ...
            $(t.cart).find("li").each(function () {
                var e = $(this), n = e.attr("data-id"), i = e.find("." + t.settings.cart + "-name").text(),
                    r = e.find("." + t.settings.cart + "-price").text(), s = e.find("input").val(),
