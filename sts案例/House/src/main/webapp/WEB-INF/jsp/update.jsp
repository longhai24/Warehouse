...
<body>
	...
	<DIV id=regLogin class=wrap>
		<DIV class=dialog>
			...
			<DIV class=box>
				<FORM id=add_action method=post name=add.action action=HouseAction?method=update enctype="multipart/form-data">
					<DIV class=infos>
						<TABLE class=field>
							<TBODY>
								<TR>
								...
								<TR>
									...
									<TD>省份：<SELECT class=text name=pro id=pro><option>请选择</option></SELECT>
										市区：<SELECT class=text name=dow id=dow></SELECT>街道：<SELECT
										class=text name=str id=str></SELECT>
									</TD>
								</TR>
								<TR>
...
$.ajax({
	...
	success: function (data) {
	  ...
	  var myhouse = window.sessionStorage.getItem("myhouse");
	  var obj = eval("("+myhouse+")");
	  $("#add_action_title").val(obj.house_title)
	  $("#type").val(obj.type_id)
	  $("#add_action_floorage").val(obj.house_area)
	  $("#add_action_price").val(obj.house_price)
	  $("#add_action_contact").val(obj.house_phone)
	  $("#description").val(obj.house_info)
