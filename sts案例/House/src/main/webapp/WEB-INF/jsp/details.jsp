...
<body>
		...
		<DIV class=search>
			<FORM method=get>
				<INPUT class=text type=text name=keywords> <LABEL
					class="ui-green searchs"><a href="HouseAction?method=showAllHouse" target="_blank">搜索房屋</a></LABEL>
			</FORM>
		</DIV>
	</DIV>
	...
	<DIV id=view class="main wrap">
		<DIV class=intro>
			<DIV class=lefter>
				<H1>${show.htil }</H1>
				<DIV class=subinfo>${show.hdate }</DIV>
				<DIV class=houseinfo>
					<P>
						户 型：<SPAN>${show.tname }</SPAN>
					</P>
					<P>
						面 积：<SPAN>${show.hare }m<SUP>2</SUP></SPAN>
					</P>
					<P>
						位 置：<SPAN>${show.pname }${show.dname }${show.sname }</SPAN>
					</P>
					<P>
						联系方式：<SPAN>${show.hphone }</SPAN>
					</P>
				</DIV>
			</DIV>
			<DIV class=side>
