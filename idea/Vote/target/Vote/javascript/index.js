...
$(document).ready(function() {
    ...
    $("#copy-to-clipboard").on("click", function() {
        ...
        //$modal.find(":input:first").text($(".ibox-content").html());
        $modal.find(":input:first").select().focus();
        $("xmp.form-control").text($(".ibox-content").html().trim());
        //$(".modal-open").text($(".ibox-content").html())
        return false
    })
    //点击添加表单
    $(".btntext,.btntexts").click(function() {
        //alert("1进入tableList方法 根据id生成对应控件 追加到.ui-sortable");
...
function tableList(id) {
    ...
    switch (id) {
        case "text":
            //文本框name=text
            var text = '<input name="text11" type="text" class="form-control" placeholder="请输入文本">';
        ...
        case "select":
            //下拉框name=drop-down
            var select = '<select class="form-control" name="drop-down"><option>请选择</option><option>默认</option></select>';
        ...
        case "textarea":
            //多行文本name=multi-line
            var textarea = '<textarea name="multi-line" class="form-control" placeholder="请输入文本"></textarea>';
        ...
        case "radio":
            ...
            var radio = '<label  class="radio-inline"><input name="radio" type="radio" value="默认1" name="rad' + radom + '" checked> 默认1</label><label class="radio-inline"><input type="radio" name="rad' + radom + '" value="默认2"> 默认2</label>';
        ...
        case "checkbox":
            //多选框 name=multiple
            var checkbox = '<label class="radio-inline" style="padding-left:0px;"><input name="multiple" type="checkbox" name="默认1"> 默认1</label><label class="radio-inline" style="padding-left:0px;"><input type="checkbox" name="默认2"> 默认2</label>';
        ...
        case "datetime":
            //时间 name=time
            var datetime = '<input name="time" readonly="" class="form-control form_datetime" type="text" style="border: 1px solid #e5e6e7;">';
        ...
        case "file":
            //上传 name=uploading
            var file = '<input name="uploading" class="file uploadfile" type="file" multiple data-min-file-count="1">';
...
function tabUp(tabL, $el) {
    ...
    switch (tabL) {
        ...
        case "select":
            ...
            content += '<div class="col-sm-12" style="margin-bottom:10px"><label class="col-sm-3 control-label">属性名：</label><div class="col-sm-9"><input type="text" class="form-control" value="' + $el.find("input:eq(0)").attr("placeholder") + '" placeholder="请输入属性名"></div></div></div>';
            break;
        case "textarea":
            //多行文本
    ...
    $modal.find(".btn-success").click(function(ev2) {
        ...
        switch (tabname) {
            case "text":
                ...
                $el.find("input:eq(0)").attr("placeholder", $(this).parent().find("input:eq(1)").val()).attr("name",$(this).parent().find("input:last").val());
                $modal.modal("hide");
                return false;
                break;
            case "select":
                ...
                if (panduan != 0) {
                    ...
                    $el.find("select").html(option).attr("name",$(this).parent().find("input:last").val());
                    $modal.modal("hide");
                }
                break;
            case "textarea":
                //多行文本
                $el.find("label:eq(0)").html($(this).parent().find("input:eq(0)").val() + "：");
                $el.find("textarea").attr("placeholder", $(this).parent().find("input:eq(1)").val()).attr("name",$(this).parent().find("input:last").val());
                $modal.modal("hide");
                return false;
                break;
            case "datetime":
                ...
                $el.find("label:eq(0)").html($(this).parent().find("input:eq(0)").val() + "：");
                $modal.modal("hide");
                $el.find("input:eq(0)").attr("name",$(this).parent().find("input:last").val());
                return false;
                break;
            case "file":
                //上传
                ...
                $(this).parent().find("input").each(function(index, element) {
                    if (index == 0) {
                        ...
                        if ($(this).val() == "") {
                            ...
                            option += '<label class="radio-inline"><input type="radio" name="' + radom + '" value="' + $(this).val() + '" checked=""> ' + $(this).val() + '</label>';
                        }
                    } else {
                        if ($(this).val() == "") {random
                            return false;
                        } else {
                            option += '<label class="radio-inline"><input type="radio" name="' + radom + '" value="' + $(this).val() + '" > ' + $(this).val() + '</label>';
                        }
                    }
                });
                if (panduan != 0) {
                    ...
                    th=$(this);
                    ele=$("<div>").html(option);
                    $.each(ele.find("input"),function(i,obj){
                        $(obj).attr("name",th.parent().find("input:last").val());
                    });
                    ele.find("label:last").remove();
                    option=ele.html();
